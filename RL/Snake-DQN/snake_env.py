    ...
    def get_state(self):
        ...
        if self.env_info['state_space'] == 'coordinates':
            ...
            state = [int(self.snake.y < self.apple.y), int(self.snake.x < self.apple.x), int(self.snake.y > self.apple.y), int(self.snake.x > self.apple.x), \
                    ...
                    int(self.snake.direction == 'up'), int(self.snake.direction == 'right'), int(self.snake.direction == 'down'), int(self.snake.direction == 'left')]
        elif self.env_info['state_space'] == 'image':
            state = np.array(
                Image.open(
                    io.BytesIO(
                        turtle.getscreen().getcanvas().postscript(colormode="color").encode("utf-8")
                        )
                    ).resize((80,80))
                )
        elif self.env_info['state_space'] == "both":
            image = np.array(
                Image.open(
                    io.BytesIO(
                        turtle.getscreen().getcanvas().postscript(colormode="color").encode("utf-8")
            ...
            engineered_features = np.array([int(self.snake.y < self.apple.y), int(self.snake.x < self.apple.x), int(self.snake.y > self.apple.y), int(self.snake.x > self.apple.x), \
                    int(wall_up or body_up), int(wall_right or body_right), int(wall_down or body_down), int(wall_left or body_left), \
                    int(self.snake.direction == 'up'), int(self.snake.direction == 'right'), int(self.snake.direction == 'down'), int(self.snake.direction == 'left')])
            state = [image, engineered_features]
        else:
            state = [int(self.snake.y < self.apple.y), int(self.snake.x < self.apple.x), int(self.snake.y > self.apple.y), int(self.snake.x > self.apple.x), \
                    int(wall_up or body_up), int(wall_right or body_right), int(wall_down or body_down), int(wall_left or body_left), \
                    int(self.snake.direction == 'up'), int(self.snake.direction == 'right'), int(self.snake.direction == 'down'), int(self.snake.direction == 'left')]
